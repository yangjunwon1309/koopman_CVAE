...
class OnlineConfig:
    ...
    q_wm_penalty_latent_weight: float = 1.0
    q_wm_error_horizon:       int   = 4
    q_world_horizon:          int   = 3
...
def skill_progress_bins_from_ids(skill_ids: np.ndarray, n_bins: int) -> np.ndarray:
    """Segment-local progress bins from contiguous skill-id runs."""
    ids = np.asarray(skill_ids).reshape(-1)
    P = max(1, int(n_bins))
    out = np.zeros(ids.shape[0], dtype=np.int64)
    i = 0
    while i < ids.shape[0]:
        j = i + 1
        while j < ids.shape[0] and ids[j] == ids[i]:
            j += 1
        span = max(1, j - i)
        for t in range(i, j):
            out[t] = min(P - 1, int(((t - i) * P) / span))
        i = j
    return out
...
class PriorReplayBuffer:
    ...
    def _init(self, z_dim: int, u_dim: int):
        ...
        self._d = {
            ...
            'progress_bin': np.zeros(C, dtype=np.float32),
    ...
    def add(self, z, u, r, z_next, done,
        ...
        self._d['progress_bin'][p] = float(progress_bin)
...
class PolicyPriorOnlineTrainer:
    def __init__(self, cfg: OnlineConfig, model: KoopmanCVAE, device: str,
        ...
        self.wm_penalty_thresholds: Optional[np.ndarray] = None
        self.wm_penalty_global_threshold: float = 0.0
    ...
    def add_elite_episode(
        ...
        n_step = max(1, int(
            self.cfg.q_world_horizon
            if self.cfg.q_world_target else self.cfg.q_n_step))
    ...
    @torch.no_grad()
    def _posterior_z_from_x(self, x: torch.Tensor,
                            h: torch.Tensor) -> torch.Tensor:
        if x.dim() == 1:
            x = x.unsqueeze(0)
        if h.dim() == 1:
            h = h.unsqueeze(0)
        z, _ = self.model.posterior(x, h)
        return z

    @torch.no_grad()
    def _wm_rollout_error_sequence(
        self,
        z_seq: torch.Tensor,
        h_seq: torch.Tensor,
        ...
        """Dense H-step WM rollout error from each t over latent, q and object p."""
        H = int(self.cfg.q_wm_error_horizon if H is None else H)
        L = int(min(z_seq.shape[0], h_seq.shape[0], actions.shape[0], x_seq.shape[0]))
        if L <= 1:
            return np.zeros(0, dtype=np.float32)
        m = self.model
        n0 = L - 1
        z_roll = z_seq[:n0].detach()
        h_roll = h_seq[:n0].detach()
        total = z_roll.new_zeros(n0)
        count = z_roll.new_zeros(n0)
        for k in range(H):
            n = n0 - k
            if n <= 0:
                break
            a_k = actions[k:k + n].to(z_roll.device)
            u_k = m.action_encoder(a_k)
            z_cur = z_roll[:n]
            h_cur = h_roll[:n]
            w = torch.softmax(m.skill_prior(h_cur), dim=-1)
            z_pred, _, _ = m.koopman(z_cur, u_k, w)
            recon = m.decoder(z_pred)
            q_pred = symexp(recon['q'])
            p_pred = symexp(recon['delta_p'])
            x_true = x_seq[k + 1:k + 1 + n].to(z_roll.device)
            z_true = z_seq[k + 1:k + 1 + n].to(z_roll.device)
            q_true = x_true[..., X_DQ_START:X_DQ_END]
            p_true = x_true[..., X_DP_START:X_DP_END]
            z_mse = (z_pred - z_true).pow(2).mean(-1)
            q_mse = (q_pred - q_true).pow(2).mean(-1)
            p_mse = (p_pred - p_true).pow(2).mean(-1)
            err = (
            ...
            total[:n] += err
            count[:n] += 1.0
            h_next = m.recurrent(h_cur, z_cur, a_k)
            z_roll = z_roll.clone()
            h_roll = h_roll.clone()
            z_roll[:n] = z_pred
            h_roll[:n] = h_next
        return (total / count.clamp_min(1.0)).detach().cpu().numpy().astype(np.float32)
    ...
    def build_elite_wm_thresholds(self, force: bool = False) -> Optional[np.ndarray]:
        if self.cfg.lambda_q_wm_penalty <= 0.0 or self.elite_buf is None:
            return None
        if self.wm_penalty_thresholds is not None and not force:
            return self.wm_penalty_thresholds
        ...
        K = int(self.model.cfg.num_skills)
        P = max(1, int(self.cfg.q_wm_progress_bins))
        thresholds = np.full((K, P), global_thr, dtype=np.float32)
        skill_ids = self.elite_buf._d.get('skill_id', None)
        progress_bins = self.elite_buf._d.get('progress_bin', None)
        if skill_ids is not None and progress_bins is not None:
            s = np.clip(skill_ids[:n].astype(np.int64), 0, K - 1)
            p = np.clip(progress_bins[:n].astype(np.int64), 0, P - 1)
            min_count = max(1, int(self.cfg.q_wm_min_bin_count))
            for k in range(K):
                for b in range(P):
    ...
    def _elite_wm_penalty_threshold(self) -> Optional[float]:
        thresholds = self.build_elite_wm_thresholds(force=False)
        if thresholds is None:
            return None
        return float(self.wm_penalty_global_threshold)
    ...
    def _q_wm_penalty(
        ...
        thresholds = self.build_elite_wm_thresholds(force=False)
        if thresholds is None or 'wm_err' not in batch:
            return like.new_zeros(like.shape)
        wm_err = batch['wm_err'].to(like.device)
        K = int(self.model.cfg.num_skills)
        P = max(1, int(self.cfg.q_wm_progress_bins))
        skill_id = batch.get('skill_id', like.new_zeros(like.shape)).long()
        progress_bin = batch.get('progress_bin', like.new_zeros(like.shape)).long()
    ...
    def _effective_pi_q_weight(self) -> float:
        if self.step < int(self.cfg.pi_update_start):
            return 0.0
    ...
    def _update_skill_arg_q_pi(
        ...
        with torch.no_grad():
            ...
            if self.cfg.q_world_target:
                r_targ = b_q.get('r_return', r).to(z.device).clamp(
                    m.cfg.v_min, m.cfg.v_max)
                z_boot = b_q.get('z_boot', z_next)
                h_boot = b_q.get('h_boot', h_next_q)
                default_discount = (
                    (1 - done)
                    * (float(self.cfg.q_n_step_gamma)
                       ** max(1, int(self.cfg.q_world_horizon)))
                )
                discount = b_q.get('discount', default_discount).to(z.device)
            else:
        ...
        if self.cfg.q_world_target:
            ...
            loss_q_negative = zero
            loss_q_success_rank = zero
            loss_q = loss_q_td
        else:
    ...
    def save(self, path: str):
        ...
        torch.save({
            ...
            'wm_penalty_global_threshold': self.wm_penalty_global_threshold,
            'wm_penalty_thresholds': self.wm_penalty_thresholds,
        }, path)
        print(f"  Saved: {path}")
    ...
    def load(self, path: str) -> int:
        ...
        self.wm_penalty_global_threshold = float(
            ck.get('wm_penalty_global_threshold',
                   self.wm_penalty_global_threshold))
        self.wm_penalty_thresholds = ck.get(
            'wm_penalty_thresholds', self.wm_penalty_thresholds)
...
def prefill_prior_buffer_from_offline(trainer: PolicyPriorOnlineTrainer,
    ...
    for ep in episodes:
        ...
        if L > 1:
            wm_err_step = trainer._wm_rollout_error_sequence(
                z_ep, h_pre_ep,
                torch.FloatTensor(acts).to(dev),
                torch.FloatTensor(x_ep).to(dev),
                H=trainer.cfg.q_wm_error_horizon,
            )
        else:
            wm_err_step = np.zeros(0, dtype=np.float32)
        with torch.no_grad():
            skill_probs_ep = torch.softmax(model.skill_prior(h_pre_ep), dim=-1)
            skill_ids_ep = skill_probs_ep.argmax(dim=-1).detach().cpu().numpy()
        progress_bins_ep = skill_progress_bins_from_ids(
            skill_ids_ep, trainer.cfg.q_wm_progress_bins)
        ...
        n_step = max(1, int(
            trainer.cfg.q_world_horizon
            if trainer.cfg.q_world_target else trainer.cfg.q_n_step))
        ...
        for t in range(L - 1):
            ...
            for j in range(n_avail):
                r_ret += (gam ** j) * float(r_step[t + j])
            boot_idx = min(t + n_avail, L - 1)
            done_n = boot_idx >= (L - 1)
            wm_err = float(wm_err_step[t]) if t < len(wm_err_step) else 0.0
            trans = {
                'z': z_ep[t].cpu().numpy(),
                ...
                'has_skill_arg': 1.0,
                'wm_err': wm_err,
                'progress_bin': int(progress_bins_ep[t]),
                'source': 1.0,
            }
...
def train_policy_prior_online(cfg: OnlineConfig,
    ...
    print(f"  q wm_penalty lambda={cfg.lambda_q_wm_penalty:.3f} "
          ...
          f"{cfg.q_wm_penalty_q_weight:.2f}/"
          f"{cfg.q_wm_penalty_obj_weight:.2f}")
    ...
    if cfg.pi_q_guide_loss_threshold > 0.0:
        print(
            "  phase P1=elite BC + Q warmup(QW=0) until "
        ...
        print(
            "  phase P1=elite BC from start | "
            f"P2=Q guide after pi_start(QW={cfg.pi_q_weight:.3f})"
        )
    ...
    while global_step < cfg.n_env_steps:
        ...
        if cfg.actor_mode == 'skill_arg':
            ...
            x_roll: List[np.ndarray] = [ctx._obs_to_x(obs).copy()]
            z_after: List[np.ndarray] = []
            h_after: List[np.ndarray] = []
            z_final = None
            h_final = None
            done_chunk = False
            ...
            for a_np, u_np in zip(a_seq_np, u_seq_np):
                ...
                exec_rewards.append(float(max(0.0, r_env)))
                x_roll.append(np.asarray(x_next_np, dtype=np.float32).copy())
                nxt = ctx.current_latent()
                if nxt is not None:
                    z_next_t, h_next_t = nxt
                    z_final = z_next_t.cpu().numpy()[0].copy()
                    h_final = h_next_t.cpu().numpy()[0].copy()
                    z_after.append(z_final.copy())
                    h_after.append(h_final.copy())
                obs = obs_nx
                global_step += 1
                trainer.step = global_step
            ...
            if exec_actions and z_final is not None:
                ...
                H_target = min(max(1, int(cfg.q_world_horizon)), H_exec)
                r_ret = sum(
                    (cfg.q_n_step_gamma ** j) * exec_rewards[j]
                    for j in range(H_target)
                )
                z_boot_store = z_after[H_target - 1] if z_after else z_final
                h_boot_store = h_after[H_target - 1] if h_after else h_final
                z_next_store = z_after[0] if z_after else z_final
                h_next_store = h_after[0] if h_after else h_final
                chunk_mask = np.ones(H_exec, dtype=np.float32)
                wm_err_chunk = float(np.max(exec_wm_errors)) if exec_wm_errors else 0.0
                progress_bin = 0
                if len(x_roll) >= 2:
                    x_arr = np.asarray(x_roll, dtype=np.float32)
                    a_arr = np.zeros(
                        (len(x_arr), trainer.model.cfg.action_dim),
                        dtype=np.float32)
                    a_arr[:H_exec] = np.asarray(exec_actions, dtype=np.float32)
                    with torch.no_grad():
                        enc_exec = trainer.model.encode_sequence(
                            torch.FloatTensor(x_arr).unsqueeze(0).to(dev),
                            torch.FloatTensor(a_arr).unsqueeze(0).to(dev))
                        z_exec_seq = enc_exec['o_seq'][0]
                        h_exec_seq = enc_exec.get(
                            'h_pre_seq', enc_exec['h_seq'])[0]
                        wm_seq = trainer._wm_rollout_error_sequence(
                            z_exec_seq, h_exec_seq,
                            torch.FloatTensor(a_arr).to(dev),
                            torch.FloatTensor(x_arr).to(dev),
                            H=trainer.cfg.q_wm_error_horizon)
                        if len(wm_seq) > 0:
                            wm_err_chunk = float(wm_seq[0])
                        skill_ids_exec = torch.softmax(
                            trainer.model.skill_prior(h_exec_seq),
                            dim=-1).argmax(dim=-1).detach().cpu().numpy()
                    progress_bin = int(skill_progress_bins_from_ids(
                        skill_ids_exec, trainer.cfg.q_wm_progress_bins)[0])
                trans = {
                    'z': z_start,
                    'h': h_start,
                    'u': exec_us[0],
                    ...
                    'h_boot': h_boot_store,
                    'discount': float(
                        0.0 if (done_chunk and H_exec <= H_target)
                        else cfg.q_n_step_gamma ** H_target),
    ...
    if args.prior_online:
        ...
        if not args.no_prefill:
            ...
            prefill_prior_buffer_from_offline(
                ...
                env_name=args.env)
            trainer.build_elite_wm_thresholds(force=True)
